...
<section class="content-header listing-content-header">
	...
	<ol class="breadcrumb hidden-sm hidden-xs">
		...
		<li>
			<a href="/{{Auth::user()->type()}}/hub_listings">
				<i class="fa fa-user-circle"></i> Донации</a>
		</li>
		<li>
			<a href="/{{Auth::user()->type()}}/hub_listings/edit/"{{$hub_listing->id}}>Измени</a>
		</li>
	</ol>
</section>
...
<section class="content listing-content edit-listing-content">
	...
	<form id="edit-listing-form" class="" action="{{ route('admin.edit_hub_listing',$hub_listing->id) }}" method="post" enctype="multipart/form-data">
		<input type="hidden" name="hub_listing_id" value="{{$hub_listing->id}}">
            {{ csrf_field() }}
		<div class="listing-box-body-wrapper">
			<div class="box-body">
				...
				<div class="form-group{{ $errors->has('food_type') ? ' has-error' : '' }} row">
					...
					<div class="col-md-6">
						<select id="food_type_select_admin_hub" class="form-control" name="food_type" required>
							@foreach ($food_types as $food_type)
								<option value={{$food_type->id}}
									{{ (old('food_type') == $food_type->id) ? ' selected' : (($hub_listing->product->food_type->id == $food_type->id) ? ' selected' : '')}}>{{$food_type->name}}</option>
							@endforeach
						</select>
				...
				<div class="form-group{{ $errors->has('product_id') ? ' has-error' : '' }} row">
					...
					<div class="col-md-6">
						<select id="product_id_select_admin_hub" class="form-control" name="product_id" required>
						@foreach ($products as $product)
							<option value={{$product->id}}{{ (old('product_id') == $product->id) ? ' selected' : (($hub_listing->product->id == $product->id) ? ' selected' : '')}}>{{$product->name}}</option>
						@endforeach
					</select>
				...
				<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }} row">
					...
					<div class="col-md-6">
						<textarea rows="4" form="edit-listing-form" id="description" class="form-control"
											name="description" >{{ (old('description')) ? old('description') : $hub_listing->description }}</textarea>
					...
					<div class="form-group{{ $errors->has('quantity') ? ' has-error' : '' }} row">
						...
						<div class="col-md-6">
							<div class="col-xs-6" style="padding-left: 0;">
								<input type="number" id="quantity" name="quantity" class="form-control"
											...
											value="{{ (old('quantity')) ? old('quantity') : $hub_listing->quantity }}" >
							...
							<div class="col-xs-6" style="padding-right: 0;">
								<select id="quantity_type_admin_hub" class="form-control" name="quantity_type">
									{{-- <option value="">-- Избери --</option> --}}
									@foreach ($hub_listing->product->quantity_types as $quantity_type)
										<option value={{$quantity_type->id}}
											{{ (old('quantity_type') == $quantity_type->id) ? ' selected' : (($hub_listing->quantity_type->id == $quantity_type->id) ? ' selected' : '')}}>{{$quantity_type->description}}</option>
									@endforeach
								</select>
						...
						<div class="form-group{{ $errors->has('date_listed') ? ' has-error' : '' }} row">
							...
							<div class="col-md-6">
									...
									value="{{(old('date_listed')) ? old('date_listed') : Methods::convert_date_input_from_db($hub_listing->date_listed)}}" style="text-align: center;" required >
						...
						<div class="form-group{{ $errors->has('sell_by_date') ? ' has-error' : '' }} row">
							...
							<div class="col-md-6">
								<input id="sell_by_date" type="date" class="form-control" name="sell_by_date" max="9999-01-01"
											value="{{(old('sell_by_date')) ? old('sell_by_date') : $hub_listing->sell_by_date}}" style="text-align: center;" required >
						...
						<div class="col-md-6">
							<input id="date_expires" type="datetime-local" class="form-control" name="date_expires"
								value="{{(old('date_expires')) ? old('date_expires') : Methods::convert_date_input_from_db($hub_listing->date_expires)}}" style="text-align: center;" required >
							...
							<div class="col-md-6">
								<div class="col-xs-6"  style="padding-left: 0;">
									<input id="pickup_time_from" type="time" step="3600" name="pickup_time_from" class="form-control"
												value="{{ (old('pickup_time_from')) ? old('pickup_time_from') : $hub_listing->pickup_time_from }}" >
								</div>
							...
							<div class="col-md-6">
								<div class="col-xs-6"  style="padding-left: 0;">
									<input id="pickup_time_to" type="time" step="3600" name="pickup_time_to" class="form-control"
												value="{{ (old('pickup_time_to')) ? old('pickup_time_to') : $hub_listing->pickup_time_to }}" >
								</div>
								<div class="col-xs-6" style="padding-right: 0px;">
									<span>часот</span>
								</div>
