...
<section class="content active-listings-content">
  ...
  <section class="filter donations-filter">
  	<div class="filter-wrapper row">
    	<form id="donations-filter-form" class="donations-filter-form" action="{{route('admin.hub_listings')}}" method="post">
  ...
  @foreach ($hub_listings->get() as $hub_listing)
        ...
        <div id="listingbox{{$hub_listing->id}}" name="listingbox{{$hub_listing->id}}"></div>
          ...
          <div class="{{($selected_filter == "active") ? "hub-my-active-listings-box" : "hub-my-past-listings-box"}}
                box listing-box listing-box-{{$hub_listing->id}} {{($hub_listing->id == old('hub_listing_id')) ? 'box-error' : 'collapsed-box' }}">
                ...
                <div class="header-wrapper">
                  <div id="listing-title-{{$hub_listing->id}}" class="listing-title col-xs-12">
                    <div class="col-md-10 col-xs-12 hub-listing-title-panel panel">
                      <strong>Хаб: {{$hub_listing->hub->first_name}} {{$hub_listing->hub->last_name}} | {{$hub_listing->hub->organization->name}}</strong>
                      <br>
                      <span>{{$hub_listing->product->food_type->name}} | {{$hub_listing->product->name}}</span>
                    </div>
                    <div class="col-md-2 col-xs-12">
                      <button type="button" id="hub-listing-details-{{$hub_listing->id}}"
                            class="btn btn-primary pull-right admin-hub-listing-button hub-listing-details hub-listing-details-{{$hub_listing->id}}"
                            name="button" data-toggle="modal" data-target="#hub-listing-details-popup">Детали...</button>

                      @if ($selected_filter == "active")
                        <a href="{{route('admin.edit_hub_listing', $hub_listing->id)}}"
                          id="admin-hub-listing-edit-{{$hub_listing->id}}"
                          name="button"
                          class="btn btn-success pull-right admin-hub-listing-button admin-hub-listing-edit-{{$hub_listing->id}}">Измени</a>
                      <button type="button" id="admin-hub-listing-delete-{{$hub_listing->id}}"
                            class="btn btn-danger pull-right admin-hub-listing-button admin-hub-listing-delete admin-hub-listing-delete-{{$hub_listing->id}}"
                            name="button" data-toggle="modal"
                            data-target="#delete-hub-listing-popup"
                            title="{{($hub_listing->listing_offers_count) ? 'Донацијата не може да биде избришана се додека постојат прифаќања од приматели' : ''}}"
                            {{($hub_listing->listing_offers_count) ? ' disabled' : ''}}>Избриши</button>
                      @endif
                    </div>
                      ...
                      @switch($hub_listing->listing_offers_count)
                          ...
                          @foreach ($hub_listing->listing_offers as $listing_offer)
                            @if ($listing_offer->offer_status == 'active')
                              <a href="{{route('admin.cso_listing_offer', $listing_offer->id)}}" id="hub-listing-offer-button-{{$listing_offer->id}}" class=" hub-listing-offer-button hub-listing-offer-button-{{$listing_offer->id}}">
...
@else
  ...
  <!-- Delete Modal  -->
  <div id="delete-hub-listing-popup" class="modal fade" role="dialog">
  	<div class="modal-dialog">

  		<!-- Modal content-->
  		<div class="modal-content">
  			<form id="delete-hub-listing-form" class="delete-hub-listing-form" action="{{ route('admin.hub_listings') }}" method="post">
          <input type="hidden" name="post-type" value="delete" />
  				...
  				<div id="delete-hub-listing-body" class="modal-body delete-hub-listing-body">
  					<!-- Form content-->
  					<h5 id="popup-info" class="popup-info row italic">
  				...
  				<div class="modal-footer">
  					<input type="hidden" name="post-type" value="delete" />
  					<input type="submit" name="delete-hub-listing-popup" class="btn btn-danger" value="Избриши" />
  					<button type="button" class="btn btn-default" data-dismiss="modal">Откажи</button>
  				</div>
  			</form>
  		</div>
