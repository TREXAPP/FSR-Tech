...
@section('content')
  ...
  <section class="content-header accepted-listings-content-header">
    ...
    <ol class="breadcrumb hidden-sm hidden-xs">
      ...
      <li><a href="/{{Auth::user()->type()}}/listing_offer/{{$listing_offer->id}}"><i class="fa fa-bookmark"></i> {{$listing_offer->hub_listing->product->name}}</a></li>
    </ol>
  </section>
...
<section class="content accepted-listings-content">
          ...
          <div class="header-elements-wrapper">
            ...
            <div class="col-md-6">
              <div class="col-md-6">
                ...
                <span class="col-xs-12" id="quantity-offered-{{$listing_offer->id}}"><strong>{{$listing_offer->quantity}} (од {{$listing_offer->hub_listing->quantity}}) {{$listing_offer->hub_listing->quantity_type->description}}</strong></span>
              </div>
              <div class="col-md-6">
          ...
          @if ($listing_offer->delivered_by_hub)
            <div class="row col-md-6">
              <div class="alert alert-info" style="margin: 25px;">
                ...
                <span>Примателот одбра хабот да му ја достави донацијата</span>
              </div>
            </div>
          @else
              ...
              @foreach ($comments->where('listing_offer_id', $listing_offer->id) as $comment)
                  ...
                  @if ($comment->sender_type == 'admin')
                    <div class="row comment-row my-comment-row">
                        ...
                        <div class="comment-header my-comment-header col-xs-12">
                          ...
                          @if($selected_filter == 'active')
                            <div id="admin-comment-controls-{{$listing_offer->id}}" class="admin-comment-controls">
                              <a href="#" id="admin-edit-comment-button-cso-{{$comment->id}}" class="admin-edit-comment-button-cso"
                                data-toggle="modal" data-target="#edit-comment-popup" ><i class="fa fa-pencil fa-1-5x"></i></a>
                                <a href="#" id="admin-delete-comment-button-cso-{{$comment->id}}" class="admin-delete-comment-button-cso"
                      ...
                      @if ($comment->sender_type == 'hub' || $comment->sender_type == 'cso')
                        <div class="row comment-row other-comment-row">
                          ...
                          <div class="comment-bubble other-comment-bubble">
                            <div class="comment-header other-comment-header col-xs-12">
                              @if ($comment->sender_type == 'hub')
                                <span class="comment-name other-comment-name">(хаб) {{$listing_offer->hub_listing->hub->first_name}} {{$listing_offer->hub_listing->hub->last_name}}</span>
                              @elseif ($comment->sender_type == 'cso')
                ...
                @if($selected_filter == 'active')
                  <div class="new-comment-wrapper">
                    <div id="new-comment-box-wrapper-{{$listing_offer->id}}" class="new-comment-box-wrapper collapse" collapsed>
                      <form class="form-group new-comment-form" action="{{ route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
                        {{csrf_field()}}
  ...
  <div id="admin-update-volunteer-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <form id="admin-update-volunteer-form" class="admin-update-volunteer-form" action="{{  route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
          {{ csrf_field() }}
  ...
  <div id="admin-delete-offer-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="delete-offer-form" class="delete-offer-form" action="{{ route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
          {{ csrf_field() }}
  ...
  <div id="delete-comment-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="delete-comment-form" class="delete-comment-form" action="{{ route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
          {{ csrf_field() }}
  ...
  <div id="edit-comment-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="edit-comment-form" class="edit-comment-form" action="{{ route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
          {{ csrf_field() }}
  ...
  <div id="edit-quantity-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="edit-quantity-form" class="edit-quantity-form" action="{{ route('admin.cso_listing_offer', $listing_offer->id) }}" method="post">
          {{ csrf_field() }}
      ...
      <div class="modal-content">
          ...
          <div id="listing-confirm-body" class="modal-body listing-confirm-body">
            ...
            <div id="details-popup-quantity" class="details-popup-quantity popup-element row">
              ...
              <div id="details-popup-quantity-value" class="details-popup-quantity-value popup-element-value col-xs-6">{{$listing_offer->hub_listing->quantity}} {{$listing_offer->hub_listing->quantity_type->description}}</div>
            </div>
            ...
            <div id="details-popup-pickup-time" class="details-popup-pickup-time popup-element row">
              ...
              <div id="details-popup-pickup-time-value" class="details-popup-pickup-time-value popup-element-value col-xs-6">од {{Carbon::parse($listing_offer->hub_listing->pickup_time_from)->format('H:i')}} до {{Carbon::parse($listing_offer->hub_listing->pickup_time_to)->format('H:i')}} часот</div>
            </div>
            ...
            <div id="details-popup-listed" class="details-popup-listed popup-element row">
              ...
              @if ($selected_filter == 'active')
                <div id="details-popup-listed-value" class="details-popup-listed-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->date_listed)->diffForHumans()}}</div>
              @else
                <div id="details-popup-listed-value" class="details-popup-listed-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->date_listed)->format('d-m-Y')}}</div>
              @endif
            </div>
            ...
            <div id="details-popup-sell-by-date" class="details-popup-sell-by-date popup-element row">
              ...
              @if ($selected_filter == 'active')
                <div id="details-popup-sell-by-date-value" class="details-popup-sell-by-date-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->sell_by_date)->diffForHumans()}}</div>
              @else
                <div id="details-popup-sell-by-date-value" class="details-popup-sell-by-date-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->sell_by_date)->format('d-m-Y')}}</div>
              @endif
            </div>
            ...
            <div id="details-popup-expires-in" class="details-popup-expires-in popup-element row">
              ...
              @if ($selected_filter == 'active')
                <div id="details-popup-expires-in-value" class="details-popup-expires-in-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->date_expires)->diffForHumans()}}</div>
              @else
                <div id="details-popup-expires-in-value" class="details-popup-expires-in-value popup-element-value col-xs-6">{{Carbon::parse($listing_offer->hub_listing->date_expires)->format('d-m-Y')}}</div>
              @endif
            </div>
            ...
            <div id="details-popup-food-type" class="details-popup-food-type popup-element row">
              ...
              <div id="details-popup-food-type-value" class="details-popup-food-type-value popup-element-value col-xs-6">{{$listing_offer->hub_listing->product->food_type->name}}</div>
            </div>
            ...
            <div id="details-popup-description" class="details-popup-description popup-element row">
              ...
              <div id="details-popup-description-value" class="details-popup-description-value popup-element-value col-xs-6">{{$listing_offer->hub_listing->description}}</div>
            </div>
            <h5>
            </h5>
          <div class="modal-footer">
          ...
          @if($selected_filter == 'active')
            <a href="{{route('admin.edit_hub_listing', $listing_offer->hub_listing->id)}}"
              id="admin-listing-edit-{{$listing_offer->hub_listing->id}}"
              name="button"
              class="btn btn-success admin-listing-button admin-listing-edit-{{$listing_offer->hub_listing->id}}">Измени</a>
          @endif
            <button type="button" class="btn btn-default" data-dismiss="modal">Откажи</button>
          </div>
        </div>
