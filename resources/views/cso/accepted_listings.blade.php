...
<section class="content accepted-listings-content">
              ...
              <div class="box-footer text-center">
                ...
                <div id='comments-{{$listing_offer->id}}' class="comments-wrapper">
                  ...
                  <div class="existing-comments-wrapper my-existing-comments-wrapper">
                    <?php
                    $comments_no = $comments->where('listing_offer_id', $listing_offer->id)->count();
                    $comments_count = 0;
                    ?>
                    @if ($comments_no > config('constants.max_comments_in_active_listings'))
                      <a href="{{route('cso.accepted_listings.single_accepted_listing', $listing_offer->id) . '#comments'}}">Уште {{$comments_no - config('constants.max_comments_in_active_listings')}} коментари</a>
                    @endif
  ...
  <div id="delete-comment-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="delete-comment-form" class="delete-comment-form" action="{{ route('cso.accepted_listings') }}" method="post">
          {{ csrf_field() }}
  ...
  <div id="edit-comment-popup" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content">
        <form id="edit-comment-form" class="edit-comment-form" action="{{ route('cso.accepted_listings') }}" method="post">
          {{ csrf_field() }}
