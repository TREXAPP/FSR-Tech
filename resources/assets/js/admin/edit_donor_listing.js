$('#food_type_select_admin_donor').on('change', function() {

  //remove all from organizations select box (except the default one)
  $('#product_id_select_admin_donor').children().remove();
  $('#quantity_type_admin_donor').children().remove();
  ...
  $.post('products', {'food_type': this.value, '_token':$('meta[name="csrf-token"]').attr('content')}, function(data) {
    if (data) {
      ...
      $.each(data,function(key, value) {
        ...
        $('#product_id_select_admin_donor').append('<option value=' + value.id + '>' + value.name + '</option>');
      });
        ...
        if (data) {
          ...
          $.each(data,function(key, value) {
            if (value.pivot['default']) {
              $('#quantity_type_admin_donor').append('<option value=' + value.id + ' selected>' + value.description + '</option>');
            } else {
              $('#quantity_type_admin_donor').append('<option value=' + value.id + '>' + value.description + '</option>');
            }
          });
        }
      });
...
$('#product_id_select_admin_hub').on('change', function() {
  $('#quantity_type_admin_hub').children().remove();
  ...
  $.post('quantity_types', {'product_id': this.value, '_token':$('meta[name="csrf-token"]').attr('content')}, function(data) {
    if (data) {
      ...
      $.each(data,function(key, value) {
        if (value.pivot['default']) {
          $('#quantity_type_admin_hub').append('<option value=' + value.id + ' selected>' + value.description + '</option>');
        } else {
          $('#quantity_type_admin_hub').append('<option value=' + value.id + '>' + value.description + '</option>');
        }
      });
    }
  });
