...
class AdminToHubDonorComment extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if ($this->comments_count > 3) {
            ...
            $messages->line('<a href="' . route('hub.single_listing_offer', $this->hub_listing_offer->id) . '#comments"><div style="text-align: center;font-size: 0.8em;">(Уште ' . $comments_left . ' коментари)</div></a>');
        }
        ...
        $messages->action('Кон коментарот', route('hub.single_listing_offer', $this->hub_listing_offer->id) . '#comments');
