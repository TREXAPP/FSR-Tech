...
class HubToAdminCsoComment extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        $messages = (new MailMessage)
                ...
                ->line('Хабот ' . $this->listing_offer->hub_listing->hub->first_name . ' ' . $this->listing_offer->hub_listing->hub->last_name . ' - ' . $this->listing_offer->hub_listing->hub->organization->name . ' остави коментар на прифатената донација.')
        ...
        if ($this->comments_count > 3) {
            ...
            $messages->line('<a href="' . route('admin.cso_listing_offer', $this->listing_offer->id) . '#comments"><div style="text-align: center;font-size: 0.8em;">(Уште ' . $comments_left . ' коментари)</div></a>');
        }
      ...
      ->line('<hr>')
      ->action('Кон коментарот', route('admin.cso_listing_offer', $this->listing_offer->id) . '#comments');
