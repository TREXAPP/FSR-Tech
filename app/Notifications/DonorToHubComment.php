...
class DonorToHubComment extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        $messages = (new MailMessage)
                    ...
                    ->line($this->hub_listing_offer->listing->donor->first_name . ' ' . $this->hub_listing_offer->listing->donor->last_name . ' - ' . $this->hub_listing_offer->listing->donor->organization->name . ' остави коментар на вашата донација')
        ...
        if ($this->comments_count > 3) {
            ...
            $messages->line('<a href="' . route('hub.accepted_listings.single_accepted_listing', $this->hub_listing_offer->id) . '#comments"><div style="text-align: center;font-size: 0.8em;">(Уште ' . $comments_left . ' коментари)</div></a>');
        }
        ...
        $messages->action('Кон коментарот', route('hub.accepted_listings.single_accepted_listing', $this->hub_listing_offer->id) . '#comments');
