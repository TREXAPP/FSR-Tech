...
class EditDonorListingController extends Controller
{
    /**
    ...
    public function index(Request $request, string $listing_id_string)
    {
        ...
        if ($listing_id === null) {
            ...
            if (!$listing) {
                ...
                foreach ($listing->hub_listing_offers as $hub_listing_offers) {
                    if ($hub_listing_offers->status == 'active') {
                        $max_accepted_quantity += $hub_listing_offers->quantity;
                    }
                }
                $products = Product::where('status', 'active')
                                   ->where('food_type_id', $listing->product->food_type->id)->get();
                $food_types = FoodType::where('status', 'active')->get();
                return view('admin.edit_donor_listing')->with([
              'listing' => $listing,
